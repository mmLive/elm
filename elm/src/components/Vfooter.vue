<template>
     <footer id='footer'>
        <ul>
            <li v-for='(item,index) in nav' :class="{active:comIndex==index}" @click='indexClick(index)' :key='index'>
                <svg viewBox='0 0 22 22'>
                    <path fill="#818181" fill-rule='evenodd' v-bind:d="item.path"></path>
                </svg>
...
<script>
   ...
   export default{
    ...
    // 生命周期函数 实现刷新  底部导航不切换到首页 还是当前导航项
    created(){
      for(var i=0;i<this.nav.length;i++){
          if(this.nav[i].href == this.$route.path){
               this.currentIndex = i;
               return;
